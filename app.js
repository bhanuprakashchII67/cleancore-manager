...
window.viewInvoice=async id=>{
 ...
 try{
   ...
   $("invoicePreview").innerHTML="<div class='invoice-preview'>"+
    ...
    "<div class='inv-parties'><div><b>BILL FROM</b><p><strong>CleanCore Chemical & Cleaning</strong><br>Srinivasa Colony, Manikonda, Hyderabad, Telangana, India<br>Phone: +91 91827 25773<br>Email: "+BUSINESS_EMAIL+"</p></div><div><b>BILL TO</b><p>"+(inv.customer_business && String(inv.customer_business).trim().toLowerCase()!=="na" ? "<strong>"+esc(inv.customer_business)+"</strong><br>":"")+(inv.customer_name?"<strong>"+esc(inv.customer_name)+"</strong><br>":"")+"Phone: "+esc(inv.customer_phone||"—")+"<br>"+(inv.customer_email?"Email: "+esc(inv.customer_email)+"<br>":"")+(isGstBill?"<b>GSTIN:</b> "+esc(customerGstin)+"<br>":"")+"<span class='invoice-address-label'><b>Billing Address:</b></span><br>"+esc(billingAddress)+"<br><span class='invoice-address-label'><b>Delivery Address:</b></span><br>"+esc(deliveryAddress)+"</p></div></div>"+
