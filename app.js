...
$("loginForm").addEventListener("submit",async e=>{e.preventDefault();const {data,error}=await db.auth.signInWithPassword({email:$("loginEmail").value.trim(),password:$("loginPassword").value});if(error)return toast(error.message,false);user=data.user;enter()});
$("otpForm").addEventListener("submit",async e=>{e.preventDefault();const email=$("otpEmail").value.trim();const {error}=await db.auth.signInWithOtp({email,options:{shouldCreateUser:false}});if(error)return toast(error.message,false);$("otpBox").classList.remove("hidden");toast("OTP sent to your email.")});
$("verifyOtp").onclick=async()=>{const {data,error}=await db.auth.verifyOtp({email:$("otpEmail").value.trim(),token:$("otpCode").value.trim(),type:"email"});if(error)return toast(error.message,false);user=data.user;enter()};
