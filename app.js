const SUPABASE_URL="https://rwfamxkfqslorxcrjrp.supabase.co", SUPABASE_PUBLISHABLE_KEY="sb_publishable_tzfe2xVn6OAwF-Mh5_u_zQ_a_bAW7tO";
...
$("loginForm").addEventListener("submit",async e=>{e.preventDefault();const password=$("loginPassword").value;if(!password)return toast("Enter your admin password.",false);try{const r=await fetch(SUPABASE_URL+"/auth/v1/token?grant_type=password",{method:"POST",headers:{apikey:SUPABASE_PUBLISHABLE_KEY,"Content-Type":"application/json"},body:JSON.stringify({email:"bhanuprakashchadalawada10@gmail.com",password})});const j=await r.json().catch(()=>({}));if(!r.ok)return toast("Login failed: "+(j.error_description||j.msg||j.error||("HTTP "+r.status)),false);const {error}=await db.auth.setSession({access_token:j.access_token,refresh_token:j.refresh_token});if(error)return toast("Session failed: "+error.message,false);user=j.user;await enter()}catch(err){console.error("CleanCore login error",err);return toast("Supabase connection failed. Check network/extensions.",false)}});
